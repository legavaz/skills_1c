...
Состояние строки — в списках объектов, расшифровывается по иконке состояния (выводить колонку в список не нужно):

- `_deleted: true|false` — помечен на удаление (справочники, документы, задачи, бизнес-процессы, планы счетов и видов расчёта)
- `_posted: true|false` — документы
- `_predefined: true|false` — справочники, планы счетов и видов расчёта
- `_completed: true|false` — задачи
- `_started`, `_finished` — бизнес-процессы
- `_rowPic: '<иконка>:<N>'` — сырой идентификатор иконки, для диагностики

```js
const t = await readTable();
const doc = t.rows.find(r => r['Номер'] === 'ТД00-000005');
if (doc._deleted === true) { /* помечен на удаление */ }
```

**Отсутствие поля состояния значит «не знаю», а не `false`** — свойство может быть неприменимо (у документов нет `_predefined`) либо иконка не распознана. Поэтому `if (!row._deleted)` небезопасно: «не знаю» читается как «не помечен». Сравнивайте явно (`=== true` / `=== false`), а `undefined` считайте третьим исходом. У строк вне списков объектов (табличные части форм, списки значений) полей состояния нет вовсе. Если `_rowPic` есть, а булевых нет — пришлите его значение, для этой иконки нужна поддержка расшифровки.
